...
func getPostsChronologically(c *fiber.Ctx) error {
	...
	rows, err := dbpool.Query(c.Context(), `SELECT * FROM getPosts($1) as p ORDER BY p.id DESC LIMIT 50 OFFSET $2`, token, 50*pageInt)
...
func getUserPostsChronologically(c *fiber.Ctx) error {
	...
	rows, err := dbpool.Query(c.Context(), `SELECT * FROM getPosts($1) as p WHERE p.username = $2 ORDER BY p.id DESC LIMIT 50 OFFSET $3`, token, username, 50*pageInt)
